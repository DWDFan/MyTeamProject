...
+(void)showAdvertiserView:(NSString *)image
{
    ...
    [[AdvertiseHelper sharedInstance] getAdvertisingImage:image];
}
 ...
 *  初始化广告页面
 */
- (void)getAdvertisingImage:(NSString *)image
{
    NSArray *stringArr = [image componentsSeparatedByString:@"/"];
    ...
    if (!isExist){// 如果该图片不存在，则删除老图片，下载新图片
        [self downloadAdImageWithUrl:image imageName:imageName];
    }
}
